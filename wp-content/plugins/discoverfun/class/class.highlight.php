...
class Highlight_List_Table extends WP_List_Table {
  ...
  function column_default( $item, $column_name ) {
    ...
    function column_title($item){
      if(!current_user_can( 'manage_options' )) {   
        return $item['title'];
         ...
         $actions = array(
                'edit'      => sprintf('<a href="?page=%s&action=%s&hid=%s">Edit</a>',$_REQUEST['page'],'edit',$item['ID']),
                'delete'    => sprintf('<a href="?page=%s&action=%s&hid=%s">Delete</a>',$_REQUEST['page'],'delete',$item['ID']),
            );
    ...
    function column_cb($item) {
            return sprintf(
                '<input type="checkbox" name="highlight[]" value="%s" />', $item['ID']
            );    
        }
    ...
    function prepare_items() {
      ...
      $query = 'SELECT ID, highlight as title FROM '. $wpdb->prefix .'highlight WHERE 1=1';
      
       /* -- Searching parameters -- */
      // Search for transaction id, default search
      if (!empty($_POST['s'])) {
              $query .= ' AND (highlight  LIKE "%' . $wpdb->escape($_POST['s']) . '%")';
      }
    ...
    function process_bulk_action() {
        ...
        if( 'delete' === $this->current_action() && current_user_can( 'manage_options' )  ) {
            ...
            if ( isset($_POST['highlight']) ) {
                foreach ( $_POST['highlight'] as $id ) {
                    $sql        = 'DELETE FROM ' . $wpdb->prefix . 'highlight WHERE ID = %d';
            ...
            // else, prompts up error messge       
                ...
                echo '<div class="error msg">
                        <p>There was an error while deleting highlight. Please try again.</p>
                     </div>';
